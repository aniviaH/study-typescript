...
declare class CAnimal {}
declare class CSheep {}
declare class CDog {}
declare class CCat {}
interface Cloner {
  clone(animal: CAnimal): CAnimal;
}
interface Cloner {
  clone(animal: CSheep): CSheep;
}
interface Cloner {
  clone(animal: CDog): CDog;
  clone(animal: CCat): CCat;
}
const cloner: Cloner = {
  clone(animal){
    return animal
  }
  // Type '() => void' is not assignable to type '{ (animal: CAnimal): CAnimal; (animal: CSheep): CSheep; (animal: CDog): CDog; (animal: CCat): CCat; }'.
  // Type 'void' is not assignable to type 'CAnimal'.
}
cloner.clone(1)
...
// single string literal type bubble ===>>>
// interface Document {
//   createElement(tagName: "canvas"): HTMLCanvasElement;
